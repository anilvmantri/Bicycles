...
#define NUMWAVES 4    // Adjust for number of waves
#define WAIT 20       // Wave speed
...
void loop() {
  ...
  if ( currentTick - _previousTick >= WAIT )
  {
    ...
    for (int i = 0; i < strip.numPixels(); i++) {
      strip.setPixelColor( i, strip.Color(CIEL8[(_fade+i*(128*NUMWAVES/strip.numPixels()))%128],0, 0) );
    }
    strip.show();
